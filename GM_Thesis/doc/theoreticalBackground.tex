...
\end{align*}
, where $x \in \mathbb{R}^d$ is the decision variable vector, $l$ and $u$ denote the respective lower and upper bounds of $x$, $G_i$ are the equality constraints and $G_j$ are the inequality constraints the solution must uphold. The decision variable vector is said to exist into the \emph{decision variable space}, and the objective vector lies in the \emph{objective space}. A mapping of the feasible set under $F$ forms the \emph{attained set} $C=\{y \in \mathbb{R}^m | y=F(x), x\in \mathbb{R}^d\}$. A graphical representation of the Pareto optimal solutions creates the \emph{Pareto front}, \emph{Pareto curve}, or \emph{Pareto surface}, as shown in Figure~\ref{fig:paretoCurve}.
...
\subsection{Non-linear Constrained Optimization Problems}
line search variant
each iteration a direction dn is chosen from current location xn. Successive point relative to previous xn and selected direction. 
feasible point to another by choosing at each iteration a direction and a step size
iteratively generate sequence of points xn from i 1 to inf until termination or convergence

write in algo as in phd in mail
\subsubsection{SLSQP} \label{subsubsec:theorBack-SLSQP}

sequential least squares programming
sequential quadratic programming
non-linear optimization
obj function and constraints twice continuously differentiable
sequence of quadratic programming problems
Lagrangian l approximated by quadratic function
constraints approximated by linear hyper-space

second order approximation of objective func(lagrangian)

formalism in sequential quadratic programming
...
quasi newton : approximations of the inverse of the Hessian (see phd for formula of approximation)

feasibility of direction not guaranteed
