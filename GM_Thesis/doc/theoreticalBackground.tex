...
estimate vector known to all nodes, assumed correct if it lies on the same side of the threshold as the true global statistics vector(unknown)

decompose task to local constraints on the streams, which will guarantee a global violation hasn't occurred as long as a all local constraints hold.

convexity property -> convex hull

theorem of bounded convex hull by local constraints (balls)(proof in paper)

theorem in the monitoring task->convex hull of drift vectors and estimate vector->decomposition to balls containing convex hull
